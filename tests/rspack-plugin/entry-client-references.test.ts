 ...
 * against REAL rspack compilations — the rspack twin of
 * tests/webpack-plugin/entry-client-references.test.ts.
 *
...
const fixtureHref = (file: string): string =>
  pathToFileURL(fs.realpathSync.native(path.join(__dirname, 'fixtures', FIXTURE, file))).href;
...
describe('entryClientReferencesFilename (rspack)', () => {
  ...
  test('keeps scopes when rspack concatenates modules', () => {
    const result = run({
      isServer: true,
      extraEntries: EXTRA_ENTRIES,
      entryClientReferencesFilename: ASSET,
      configExtra: CONCATENATE_MODULES,
    });
  ...
  test('does not emit entry references when the client runtime is missing', () => {
    let error: Error | undefined;
    try {
      run({
        isServer: true,
        entryClientReferencesFilename: ASSET,
        omitRuntimeEntry: true,
      });
    } catch (e) {
      error = e as Error;
    }
